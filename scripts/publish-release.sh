...
TAG="v0.4.0"
DMG="src-tauri/target/release/bundle/dmg/CasinoSpy_0.4.0_aarch64.dmg"
cd "$(dirname "$0")/.."
...
curl -fsS -H "Authorization: token $TOKEN" -H "Content-Type: application/x-apple-diskimage" \
  --data-binary @"$DMG" \
  "https://uploads.github.com/repos/${REPO}/releases/${REL_ID}/assets?name=CasinoSpy_0.4.0_aarch64.dmg" \
